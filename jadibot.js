...
const jadibot = async (Zeroo, text, m) => {
const { sendImage, sendMessage } = Zeroo;
...
if (!Zeroo.authState.creds.registered) {
      setTimeout(async () => {
         ...
         let code = await Zeroo.requestPairingCode(phoneNumber)
         let hasilcode = code?.match(/.{1,4}/g)?.join("-") || code
         global.codepairing = `${hasilcode}`
      }, 3000)
   }
Zeroo.ws.on('CB:Blocklist', json => {
if (blocked.length > 2) return
for (let i of json[1].blocklist) {
blocked.push(i.replace('c.us','s.whatsapp.net'))}})

Zeroo.ws.on('CB:call', async (json) => {
...
let qutsnya = await Zeroo.sendContact(callerId, owner)
await Zeroo.sendMessage(callerId, { text: `Sistem Otomatis Block!!!\nJangan Menelpon Bot!!!\nSilahkan Hubungi Owner Untuk Dibuka!!!`}, { quoted : qutsnya })
await sleep(8000)
await Zeroo.updateBlockStatus(callerId, "block")
}
})
...
if (!Zeroo.public && !kay.key.fromMe && chatUpdate.type === 'notify') return
if (kay.key.id.startsWith('BAE5') && kay.key.id.length === 16) return
m = smsg(Zeroo, kay, store)
require('./XeonCheems8.js')(Zeroo, m, chatUpdate, store)
} catch (err) {
console.log(err)}
})
...
Zeroo.ev.on("connection.update", async up => {
...
if (connection == "open") {
Zeroo.id = Zeroo.decodeJid(Zeroo.user.id)
Zeroo.time = Date.now()
global.conns.push(Zeroo)
await m.reply(`*Connected to Whatsapp - Bot*\n\n*User :*\n _*× id : ${Zeroo.decodeJid(Zeroo.user.id)}*_`)
user = `${Zeroo.decodeJid(Zeroo.user.id)}`
...
console.log(`Bad Session File, Please Delete Session and Scan Again`); Zeroo.logout(); }
...
console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); Zeroo.logout(); }
else if (reason === DisconnectReason.loggedOut) { 
console.log(`Zerooce Logged Out, Please Scan Again And Run.`); Zeroo.logout(); }
else if (reason === DisconnectReason.restartRequired) { 
console.log("Restart Required, Restarting..."); start(); }
else if (reason === DisconnectReason.timedOut) { 
console.log("Connection TimedOut, Reconnecting..."); start(); }
else Zeroo.end(`Unknown DisconnectReason: ${reason}|${connection}`)
}
})
...
Zeroo.ev.on('contacts.update', update => {
for (let contact of update) {
let id = Zeroo.decodeJid(contact.id)
if (store && store.contacts) store.contacts[id] = { id, name: contact.notify }
}
})

Zeroo.getName = (jid, withoutContact  = false) => {
id = Zeroo.decodeJid(jid)
...
for (let i of kon) {
list.push({
displayName: await Zeroo.getName(i + '@s.whatsapp.net'),
vcard: `BEGIN:VCARD\n
VERSION:3.0\n
N:${await Zeroo.getName(i + '@s.whatsapp.net')}\n
FN:${await Zeroo.getName(i + '@s.whatsapp.net')}\n
item1.TEL;waid=${i}:${i}\n
...
Zeroo.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
}

Zeroo.setStatus = (status) => {
Zeroo.query({
tag: 'iq',
attrs: {
to: '@s.whatsapp.net',
