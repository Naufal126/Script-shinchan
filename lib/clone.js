...
const { smsg, sleep, getBuffer, botTerkoneksi } = require('./lib/myfunc')
    ...
    async function connectToWhatsApp() {
      ...
      client[from].ev.on('messages.upsert', async chatUpdate => {
        try {
          ...
          require('./main')(client[from], m, chatUpdate, store)
        } catch (err) {
          console.log(err)
        }
      })
